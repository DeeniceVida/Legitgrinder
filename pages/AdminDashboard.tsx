      ...
      <main className="flex-1 p-12">
        ...
        {activeTab === 'products' && (
          <div className="space-y-10">
            <div className="flex justify-between items-center bg-teal-50 px-10 py-6 rounded-full border border-teal-100">
              ...
              <span className="text-[10px] font-black uppercase text-teal-600 px-4 py-2 bg-white rounded-full">Read Only Mode</span>
            </div>

            <div className="grid grid-cols-3 gap-8">
              {products.map(p => (
                <div key={p.id} className="bg-white rounded-[3rem] p-8 border border-neutral-100 shadow-xl group">
                  <div className="h-48 rounded-[2rem] overflow-hidden mb-6 bg-gray-50">
                  ...
                  <p className="text-2xl font-black text-[#3D8593] mb-6">KES {p.priceKES.toLocaleString()}</p>
                  <button
                    onClick={() => handleDeleteProduct(p.id)}
                    className="w-full py-4 bg-rose-50 text-rose-500 rounded-2xl text-[10px] font-black uppercase hover:bg-rose-500 hover:text-white transition-all"
                  >
                    Retire Unit
                  </button>
                </div>
